<?php
    ...
    function edit_validation($data, $prefectures) {
      $err_msg = array();
      // 名前
      if (empty($data['name_sei']) || empty($data['name_mei'])) {
        $err_msg[] = '氏名を入力してください';
      ...
      // パスワード
      if((!empty($data['password']) && !empty($data['re_password'])) && (!preg_match('/\A[a-z\d]{8,20}+\z/i', $data['password']) || !preg_match('/\A[a-z\d]{8,20}+\z/i', $data['re_password']))){
        $err_msg[] = 'パスワードは8~20文字の半角英数字が使用できます';
      }
      // パスワード確認
      if ($data['password'] !== $data['re_password']) {
